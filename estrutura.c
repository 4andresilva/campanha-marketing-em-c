...
Estrutura *cria_estrutura()
{
    ...
    for (i = 0; i < TAM_NOME; i++)
        e->hash_nome[i] = cria_lista();
    for (i = 0; i < TAM_BAIRRO; i++)
        e->hash_bairro[i] = cria_lista();
    for (i = 0; i < TAM_PESSOAS; i++)
        e->hash_pessoas[i] = cria_lista();
    for (i = 0; i < TAM_CRIANCA; i++)
        e->hash_crianca[i] = cria_lista();
    for (i = 0; i < TAM_RENDA; i++)
        e->hash_renda[i] = cria_lista();
    return e;
}
...
void insere_cliente(Estrutura *e, int criterio, Cliente *c)
{
    ...
    {
    case 1:
        idx = hash_string(get_nome(c), TAM_NOME);
        insere_lista(e->hash_nome[idx], c);
        break;
    case 2:
        idx = hash_string(get_bairro(c), TAM_BAIRRO);
        insere_lista(e->hash_bairro[idx], c);
        break;
    case 3:
        idx = idx_pessoas(get_pessoas(c));
        insere_lista(e->hash_pessoas[idx], c);
        break;
    case 4:
        insere_lista(e->hash_crianca[get_criancas(c)], c);
        break;
    case 5:
        idx = faixa_renda(get_renda(c));
        insere_lista(e->hash_renda[idx], c);
        break;
    }
}
...
Lista *recupera_cliente(Estrutura *e, int criterio, int complemento, char *busca)
{
    ...
    {
    case 1:
        idx = hash_string(busca, TAM_NOME);
        return e->hash_nome[idx];
    case 2:
        idx = hash_string(busca, TAM_BAIRRO);
        return e->hash_bairro[idx];
    case 3:
        return e->hash_pessoas[idx_pessoas(complemento)];
    case 4:
        return e->hash_crianca[complemento];
    case 5:
        return e->hash_renda[complemento - 1];
    }
    return NULL;
}
...
void libera_estrutura(Estrutura *e)
{
    ...
    for (i = 0; i < TAM_PESSOAS; i++)
        libera_lista(e->hash_pessoas[i]);
    for (i = 0; i < TAM_CRIANCA; i++)
        libera_lista(e->hash_crianca[i]);
    for (i = 0; i < TAM_RENDA; i++)
        libera_lista(e->hash_renda[i]);
    free(e);
}